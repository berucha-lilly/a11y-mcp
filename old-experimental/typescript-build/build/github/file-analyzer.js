...
export class FileAnalyzer {
     ...
     */
    isLineRelevant(content, filename) {
     ...
     */
    extractCodeContext(content, targetLine, contextLines = 3) {
        ...
        return context.map((line, index) => `${lineNumbers[index].toString().padStart(3)}: ${line}`).join('\n');
    }
    /**
     * Identify the specific accessibility issue in a code change
     */
    identifyAccessibilityIssue(content, filename) {
        const trimmed = content.trim();
     ...
     */
    calculateImpactScore(file, changes) {
        let score = 0;
