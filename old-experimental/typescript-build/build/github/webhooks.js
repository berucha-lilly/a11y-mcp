...
export class WebhookHandler {
    ...
    checkRunManager;
    activeCheckRuns;
    constructor(scanner, checkRunManager) {
    ...
    async analyzePullRequest(event) {
    ...
    async getFileContent(file, event) {
        try {
    ...
    convertViolationsToGitHubFormat(violations, filePath) {
    ...
    buildCheckRun(event, violations, processingTime) {
        ...
        return {
            ...
            status: 'completed',
            conclusion: hasErrors ? 'failure' :
                hasWarnings ? 'neutral' : 'success',
            output: {
    ...
    buildAnalysisSummary(violations) {
    ...
    async postAnalysisResults(event, analysisResult) {
    ...
    async postErrorStatus(event, errorMessage) {
        const { repository, pull_request } = event;
        const [owner, repo] = repository.full_name.split('/');
        const checkRun = {
            name: 'Accessibility Review',
