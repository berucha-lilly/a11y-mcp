...
class EnhancedA11yMCPServer {
    ...
    constructor() {
        ...
        this.ldsIntegration = new LDSIntegration(process.env.LDS_STORYBOOK_URL || 'https://storybook.lilly.internal', parseInt(process.env.LDS_CACHE_TTL || '3600'));
    ...
    async handleGitHubAnalyzePR(args) {
        if (!this.githubManager) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `GitHub PR analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGitHubGetRateLimit() {
        if (!this.githubManager) {
            throw new McpError(ErrorCode.InvalidState, 'GitHub integration not configured');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get rate limit: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGitHubHealthCheck() {
        if (!this.githubManager) {
            throw new McpError(ErrorCode.InvalidState, 'GitHub integration not configured');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Health check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGitHubWebhookTest(args) {
        if (!this.githubManager) {
            throw new McpError(ErrorCode.InvalidState, 'GitHub integration not configured');
        }
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Webhook test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGitHubPostResults(args) {
    ...
    generateGitHubPRReviewPrompt(args) {
