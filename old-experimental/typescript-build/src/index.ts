...
class A11yMCPServer {
  ...
  private async handleCheckAccessibility(args: any): Promise<any> {
    try {
      ...
      throw new McpError(
        ...
        `Accessibility check failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async handleQueryLDSComponent(args: any): Promise<any> {
    try {
        ...
        content: [
          {
            ...
            ).join('\n')}\n\nAccessibility Requirements:\n${component.accessibilityRequirements.map(req => 
              `- WCAG ${req.criterion} (${req.level}): ${req.description}${req.required ? ' [Required]' : ''}`
            ).join('\n')}`
          }
        ]
      };
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `LDS component query failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async handleGetConfig(args: any): Promise<any> {
    try {
      ...
      if (section) {
        configText = JSON.stringify((config as any)[section], null, 2);
      } else {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Get config failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async handleUpdateConfig(args: any): Promise<any> {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Update config failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private getViolationCategory(violation: any): string {
  ...
  private generateFixSuggestions(violation: any, code: string, context: any): string {
    ...
    if (violation.fixSuggestions && violation.fixSuggestions.length > 0) {
      ...
      violation.fixSuggestions.forEach((suggestion: any, index: number) => {
  ...
  private generateReport(scanResult: any, format: string, includeFixes: boolean): string {
    switch (format) {
      case 'json':
  ...
  private generateMarkdownReport(scanResult: any, includeFixes: boolean): string {
    ...
    if (scanResult.summary.topCategories.length > 0) {
      ...
      scanResult.summary.topCategories.forEach((cat: any) => {
    ...
    if (scanResult.files) {
      scanResult.files.forEach((file: any) => {
        if (file.violations.length > 0) {
          ...
          file.violations.forEach((violation: any) => {
            ...
            if (includeFixes && violation.fixSuggestions.length > 0) {
              ...
              violation.fixSuggestions.forEach((suggestion: any) => {
  ...
  private generateHtmlReport(scanResult: any, includeFixes: boolean): string {
