...
export class CheckRunManager {
  ...
  constructor(apiToken: string, apiUrl: string = 'https://api.github.com') {
  ...
  private buildCheckRun(analysisResult: PRAnalysisResult): CheckRun {
  ...
  private buildAnnotations(violations: GitHubCheckViolation[]): CheckRunAnnotation[] {
    return violations.map(violation => {
      const severityToLevel = (severity: string): 'notice' | 'warning' | 'failure' => {
        switch (severity) {
  ...
  private buildViolationDetails(violation: GitHubCheckViolation): string {
  ...
  private buildSummary(summary: any, annotationCount: number): string {
