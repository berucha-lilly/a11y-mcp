...
export class WCAGRuleEngine {
    ...
    getRulesForFileType(fileType) {
        return this.getAllRules().filter(rule => rule.appliesTo.includes(fileType));
    }
    async checkFile(parser, fileType, config) {
    ...
    createAriaRequiredRule() {
        return {
            ...
            check(context) {
                ...
                const elementsWithRole = context.node.findNodesMatching((node) => {
                    return node.type === 'JSXElement' &&
                        node.openingElement?.attributes?.some((attr) => attr.name?.name === 'role');
                });
                elementsWithRole.forEach((element) => {
                    const role = element.openingElement.attributes.find((attr) => attr.name?.name === 'role')?.value?.value;
                    if (!role)
                        return;
                    ...
                    const existingAttributes = element.openingElement.attributes
                        .filter((attr) => attr.name?.name?.startsWith('aria-'))
                    ...
                    ['aria-labelledby', 'aria-describedby'].forEach(attrName => {
                        const attr = element.openingElement.attributes.find((attr) => attr.name?.name === attrName);
                        if (attr && attr.value) {
                            const ref = attr.value.value;
                            // Check if reference exists in the document
                            const referencedElements = context.node.findNodesMatching((node) => {
                                return node.type === 'JSXElement' &&
                                    node.openingElement?.attributes?.some((a) => a.name?.name === 'id' && a.value?.value === ref);
                            });
                ...
                const redundantElements = context.node.findNodesMatching((node) => {
                    if (node.type !== 'JSXElement')
                        return false;
                    const name = context.node.getJSXElementName(node);
                    const hasRole = node.openingElement?.attributes?.some((attr) => attr.name?.name === 'role');
                ...
                redundantElements.forEach((element) => {
        ...
        return {
            ...
            check(context) {
                ...
                if (context.node.findNodesMatching) {
                    const elementsWithTabindex = context.node.findNodesMatching((node) => {
                        return node.type === 'JSXElement' &&
                            node.openingElement?.attributes?.some((attr) => attr.name?.name === 'tabIndex');
                    });
                    elementsWithTabindex.forEach((element) => {
                        const tabIndexAttr = element.openingElement.attributes.find((attr) => attr.name?.name === 'tabIndex');
                ...
                if (context.node.findNodesMatching) {
                    const interactiveElements = context.node.findNodesMatching((node) => {
                        if (node.type !== 'JSXElement')
                            return false;
                        const name = context.node.getJSXElementName(node);
                        const hasClickHandler = node.openingElement?.attributes?.some((attr) => attr.name?.name === 'onClick');
    ...
    createSemanticHtmlRule() {
        return {
            ...
            check(context) {
                ...
                const allElements = context.node.findNodesMatching((node) => {
                    return node.type === 'JSXElement';
                });
                const headings = [];
                ...
                const divWithClick = context.node.findNodesMatching((node) => {
                    if (node.type !== 'JSXElement')
                        return false;
                    const name = context.node.getJSXElementName(node);
                    if (name !== 'div' && name !== 'span')
                        return false;
                    return node.openingElement?.attributes?.some((attr) => attr.name?.name === 'onClick');
                });
                divWithClick.forEach((element) => {
                    const name = context.node.getJSXElementName(element);
                    const location = context.node.getNodeLocation(element);
                    const code = context.node.getNodeCode(element);
                    violations.push({
                ...
                const listItems = context.node.findNodesMatching((node) => {
                    return node.type === 'JSXElement' &&
                        context.node.getJSXElementName(node) === 'li';
                });
                const listContainers = context.node.findNodesMatching((node) => {
                    return node.type === 'JSXElement' &&
                ...
                listItems.forEach((li) => {
                    ...
                    let inList = false;
                    context.node.findNodesMatching((node) => {
                        if (node.type !== 'JSXElement')
                            return false;
                        const name = context.node.getJSXElementName(node);
                        if (name !== 'ul' && name !== 'ol')
    ...
    createAltTextRule() {
        return {
            ...
            check(context) {
                ...
                const imgElements = context.node.findNodesMatching((node) => {
                    return node.type === 'JSXElement' &&
                        context.node.getJSXElementName(node) === 'img';
                });
                imgElements.forEach((img) => {
                    const altAttr = img.openingElement.attributes.find((attr) => attr.name?.name === 'alt');
                    const srcAttr = img.openingElement.attributes.find((attr) => attr.name?.name === 'src');
                    ...
                    if (!altAttr) {
                        ...
                        if (altText.trim() === '') {
                            ...
                            const ariaHidden = img.openingElement.attributes.find((attr) => attr.name?.name === 'aria-hidden');
                            const role = img.openingElement.attributes.find((attr) => attr.name?.name === 'role');
                            if (!ariaHidden && !role) {
                                violations.push({
                ...
                const buttonElements = context.node.findNodesMatching((node) => {
                    if (node.type !== 'JSXElement')
                        return false;
                    const name = context.node.getJSXElementName(node);
                    if (name !== 'button')
                    ...
                    const ariaLabel = node.openingElement.attributes.find((attr) => attr.name?.name === 'aria-label');
                    const ariaLabelledby = node.openingElement.attributes.find((attr) => attr.name?.name === 'aria-labelledby');
                ...
                buttonElements.forEach((button) => {
                    ...
                    const hasTextContent = button.children.some((child) => child.type === 'JSXText' && child.value.trim().length > 0);
                    if (!hasTextContent) {
                        violations.push({
...
// Helper functions
function getRequiredAriaAttributes(role) {
...
function generateAriaCodeExample(role, attributes) {
