...
export class AccessibilityScanner {
    ...
    async scanFile(filePath, content) {
        ...
        if (parseResult.hasErrors) {
            const violations = parseResult.errors.map((error, index) => ({
     ...
     */
    async scanDirectory(dirPath, ignorePatterns) {
    ...
    createFileAnalysis(filePath, fileType, content, violations, config) {
    ...
    getViolationCategory(violation) {
        ...
        if (violation.tags.includes('parse-error'))
            return 'Parse Errors';
        if (violation.tags.includes('scan-error'))
            return 'Scan Errors';
        return 'Other';
    }
    countComponents(content, fileType) {
    ...
    async checkLDSComponents(parser, filePath, config) {
        const violations = [];
        if (!parser.findNodesMatching) {
            return violations;
        }
        // Find JSX elements that might be LDS components
        const elements = parser.findNodesMatching((node) => {
    ...
    isNonStandardComponent(componentName) {
